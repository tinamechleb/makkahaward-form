...
class CmsPageController extends Controller
{
    public function index($route)
    {
        ...
        if ($page['single_record']) {
            ...
            return redirect('admin' . '/' . $route . '/' . $row['id']);
        }
        ...
        $view = view()->exists('admin/' . $route . '/index') ? 'admin/' . $route . '/index' : 'admin/cms-page/index';
        return view($view, compact('page', 'page_fields', 'rows'), ['page_title' => $page->display_name_plural]);
    }
    ...
    public function create($route)
    {
        ...
        $view = view()->exists('admin/' . $route . '/form') ? 'admin/' . $route . '/form' : 'admin/cms-page/form';
        return view($view, compact('page', 'page_fields', 'page_translatable_fields', 'extra_variables'), ['page_title' => 'Add a CMS Page']);
    }
                ...
                foreach ($translatable_fields as $field) {
                    ...
                    elseif ($field['form_field'] == 'password' || $field['form_field'] == 'password with confirmation') {
                        ...
                        if (isset($request[$locale][$field['name']]) && $request[$locale][$field['name']]) {
                            ...
                            $request[$locale][$field['name']]->move(public_path('storage/' . $request['route']), $file_name);
        ...
        foreach ($page_fields as $field) {
            ...
            elseif ($field['form_field'] == 'password' || $field['form_field'] == 'password with confirmation') {
                ...
                if ($request[$field['name']]) {
                    ...
                    $request[$field['name']]->move(public_path('storage/' . $route), $file_name);
    ...
    public function show($id, $route)
    {
        ...
        $view = view()->exists('admin/' . $route . '/show') ? 'admin/' . $route . '/show' : 'admin/cms-page/show';
        return view($view, compact('page', 'page_fields', 'translatable_fields', 'row'), ['page_title' => $page->display_name.' #'.$row->id]);
    }
    ...
    public function edit($id, $route)
    {
        ...
        $view = view()->exists('admin/' . $route . '/form') ? 'admin/' . $route . '/form' : 'admin/cms-page/form';
        return view($view, compact('page', 'page_fields', 'page_translatable_fields', 'row', 'extra_variables'), ['page_title' => 'Edit '.$page->display_name.' #'.$row->id]);
    }
    ...
    public function uploadImages(Request $request, $route)
    {
        ...
        if ($request['images']) {
            foreach ($request['images'] as $file) {
                ...
                $file->move(public_path('storage/' . $route), $file_name);
                $files[] = [
    ...
    public function destroy($id, $route)
    {
        ...
        return redirect('admin' . '/' . $route)->with('success', 'Record deleted successfully');
    }
    ...
    public function order($route)
    {
        ...
        $view = view()->exists('admin/' . $route . '/order') ? 'admin/' . $route . '/order' : 'admin/cms-page/order';
    ...
    public function changeOrder(Request $request, $route)
    {
        ...
        return redirect('admin' . '/' . $route)->with('success', 'Records ordered successfully');
    }
}